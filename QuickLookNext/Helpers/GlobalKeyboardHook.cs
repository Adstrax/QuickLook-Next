...
internal partial class GlobalKeyboardHook : IDisposable
{
    ...
    private int HookProc(int code, int wParam, ref User32.KeyboardHookStruct lParam)
    {
        ...
        // QuickLookNext itself may replay third-party hotkeys through SendInput.
        // These events are tagged in dwExtraInfo and must be forwarded without
        // entering QuickLookNext's hotkey pipeline, otherwise they can pollute state
...
{
    // Marker written to INPUT::dwExtraInfo for synthetic hotkeys replayed by QuickLookNext
    ...
    // so QuickLookNext does not treat its own replayed keystrokes as real user input.
    // This prevents false invalid-key suppression and accidental preview state changes.
    // 0x514C5452 == 'QLTR' (QuickLookNext Third-party Replay)
